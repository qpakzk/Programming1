...
#define NO 5 //88 줄의 NO가 5까지 임을 정의 
#define WORD_SIZE 100 //낱말 연습에서 낱말이 100개임을 정의
#define SHORT_SIZE 30 // 짧은 글 연습에서 글의 수가 30개임을 정의
#define LONG_SIZE 4 //긴글 연습에서 긴 글의 수가4개임을 정의
#define MAX_SIZE 100000

#define ESC 0x1B // ESC 사용을 가능하게 정의
#define DEL 0x7F // 지우는 기능을 사용 가능하게 정의

#define WORD_ROUND 20 //낱말 연습에서 20번만 연습을 할 수 있도록 정의
#define SHORT_ROUND 5 // 짧은 글 연습에서 5번만 연습을 할 수 있도록 정의
...
int menu(void);
int cal_accuracy(char *str1, char *str2, int last);// 정확도 계산을 위해 정
int cal_speed(char *input, char *sen, int idx, struct timeval *start, struct timeval *end);// 타자 속도 계산을 위
...
void exercise_word(void) {
	...
	int no;
	int match = 0;//맞힌 개수
	int progress = 0, typos = 0, accuracy = 0;//진행도, 오타수, 정확도
	int input;
	char input_buf[MAX_SIZE];
	int idx = 0, i, j;
	int selected_word[WORD_ROUND];//출력된  단어를word_round에추가 +word_round 위에 20으로 정의함
	bool duplicate; // bool함수를 이용해서 0과 1로 단어를 중복되지 않게 출력하는 것
	srand(time(NULL));

	for(i = 0; i < WORD_ROUND; i++) {
		duplicate = false; //  단어들을  출력하는 것
		while(1) {
			no = random() % WORD_SIZE; // 랜덤으로 단어 출력
			for(j = 0; j < i; j++) {//J번째 까지 돈 단어 중에 같은 것이 있는 지를 확인 하기 위함
				if(selected_word[j] == no) {
					duplicate = true;//1- 만약 J 번째 까지 돌았던 단어 중에 같은 것이 있으면 true로 출력하여 그 단어를 고르지 않도록 하는 것
					break;
				}
				else
				duplicate = false;			
		...
		memset(input_buf, 0x00, MAX_SIZE);//input_buf에 선언되어 있는 MAX_SIZE를 0으로 초기화 하기
		...
		move_cursor(0,7);
		while (1) {
			input = getchar();//우리가 입력한 문자를 받는 변수 input
			if(input == '\n') {//엔터를 눌렀을 때
				input_buf[idx] = 0;
				if(!strcmp("###", input_buf))
					return; //프로그램 실행 도중에 ###를 누르면 원래 화면으로 돌아가기
				if(strcmp(word[no], input_buf))//word[no] - 호출되어 있는 단어와 우리가 친 단어를 비교해서 두개가 동일하지 않을 때 오타수 추가 
					typos++;//오타수 추가
				else
					match++;//맞힌 수 추가 (strcmp 해석을 잘 몰라서  이부분은 잘 모르겠네요..)

				accuracy = match * 100 / (typos + match);//정확도 계산법
				progress += 5;//진행도는 하나 맞출 때마다 5씩 증가함
				idx = 0;
				break;
			}
			else
